...
export default function Navbar() {
    ...
    const authenticatedLinks = [
        {
            label: `Cart ${cart.length}`, to: "/user/cart", type: "link"
        },
        {
            label: "Account", to: "/user/account", type: "link"
        },
        {
            label: "Logout", type: "button", cb: e => {
                        ...
                        userLogin ? <>
                            {
                                authenticatedLinks.map(item => <>
                                    {
                                        item.type === "link" && <Typography variant='h6' noWrap marginX={1} >
                                            <Link to={item.to} className='nav-link'>{item.label}</Link>
                                        </Typography>
                                    }
                                    {
                                        item.type === "button" && <Typography variant='h6' noWrap >
                                            <Button variant="inherit" onClick={item.cb}>{item.label}</Button>
                                        </Typography>
                                    }
                                </>)
                            }
                            ...
                            : <Box sx={{ display: { xs: 'none', md: 'flex', alignItems: "center" } }}>
                                <Typography variant='h6' noWrap sx={{ marginRight: "1.5rem" }}>
                                    <Link to="/login" className='nav-link'>Login</Link>
                                </Typography>
                                <Typography variant='h6' noWrap sx={{ marginRight: "1.5rem" }}>
