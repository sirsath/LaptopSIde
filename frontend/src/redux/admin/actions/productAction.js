...
export const updateProductAction = ( ProductId , updatedData) => async (dispatch, getState) => {
     console.log(updatedData)
    try {
        dispatch({ type: UPDATE_PRODUCT_REQUEST })

        const { data } = await api.put(`/products/update-product/${ProductId._id}`, updatedData, {
            headers: { authorization: getState().allEmployees.employeeLogin.token }
        })
